...
from fpdf2 import FPDF  # Updated to fpdf2
...
with tab2:
    if st.session_state.roadmap:
        ...
        def generate_pdf():
            ...
            pdf.cell(w=0, h=10, txt="SkillWise Learning Roadmap", ln=True, align="C")
            pdf.set_font("Arial", size=12)
            pdf.cell(w=0, h=10, txt=f"Generated on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", ln=True, align="C")
            pdf.ln(10)
            ...
            for line in st.session_state.roadmap.splitlines():
                ...
                if line.startswith("**"):
                    ...
                    pdf.cell(w=0, h=10, txt=line[2:], ln=True)
                elif line.startswith("*"):
                    pdf.set_font("Arial", size=12)
                    pdf.cell(w=10, h=10, txt="- ", ln=0)
                    pdf.multi_cell(w=0, h=10, txt=line[1:])
                else:
                    pdf.set_font("Arial", size=12)
            ...
            pdf.cell(w=0, h=10, txt="Generated by SkillWise", ln=True, align="C")
            return pdf.output()  # fpdf2 returns bytes directly
